...
func TestIsServiceAccountEnvAvailable_NotAvailable(t *testing.T) {
	// Default path doesn't exist in test environment
	assert.False(t, IsServiceAccountEnvAvailable())
}
...
func TestClient_IsServiceAccountAvailable(t *testing.T) {
	client, err := New(t.Context(), Options{})
	require.NoError(t, err)

	// Default path doesn't exist in test environment
	assert.False(t, client.IsServiceAccountAvailable())
}
