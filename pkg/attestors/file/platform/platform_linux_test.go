...
func TestOpsInterface(t *testing.T) {
	t.Run("linux implementation", func(t *testing.T) {
		var _ Ops = (*LinuxOps)(nil)
		ops := NewLinuxOps()
		assert.NotNil(t, ops)
	})
...
// TestLinuxOps tests Linux-specific platform operations.
func TestLinuxOps(t *testing.T) {
	ops := NewLinuxOps()
	...
	t.Run("CheckFileDuplicate detects hardlinks", func(t *testing.T) {
		tmpFile, err := os.CreateTemp(t.TempDir(), "linux-file-test-*")
		require.NoError(t, err)
	...
	t.Run("ExtractOwnership from file", func(t *testing.T) {
		tmpFile, err := os.CreateTemp(t.TempDir(), "linux-ownership-test-*")
		require.NoError(t, err)
		require.NoError(t, tmpFile.Close())
		...
		require.NotNil(t, ownership, "Linux should extract ownership")
	...
	t.Run("ExtractTimestamps from file", func(t *testing.T) {
		tmpFile, err := os.CreateTemp(t.TempDir(), "linux-timestamp-test-*")
		require.NoError(t, err)
		require.NoError(t, tmpFile.Close())
		...
		require.NotNil(t, timestamps, "Linux should extract timestamps")
